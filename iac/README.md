# IaC section | CocciaBot: Telegram Bot running on PowerShell and Azure Functions

## Description

This script will configure the ```main.tf``` file in the ```/iac/deploy``` folder.

## How to use it

```prepareTerraformBackend.ps1``` is an example that shows how to use it.
...
```PowerShell
prepareTerraformBackend.ps1 -MainFilePath "..\deploy\main.tf"
```

After script execution you will see an ```Output``` folder within a new ```main.tf``` file within the configuration to store Terraform backend on Azure. As shown in the next screenshot.

![Output folder - screenshot example](https://github.com/AngelusGi/CocciaBot/blob/master/Screenshot/iac/output-screenshot.png?raw=true)
